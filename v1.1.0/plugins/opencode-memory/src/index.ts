...
interface MemoryConfig {
  ...
  strict: {
    forbiddenCommands: ForbiddenEntry[];
    rules: string[];
  };
...
interface FragmentWithAssocs extends Fragment {
  associations_arr: string[];
  matchScore?: number;
}

function generateAssociations(content: string, maxCount: number = 5): string[] {
  return [content];
}
...
class MemoryFragmentDB {
  ...
  private generateId(prefix = "frag"): string {
    return `${prefix}_${Date.now().toString(36)}_${Math.random().toString(36).substr(2, 5)}`;
  }

  private parseFragment(row: any): FragmentWithAssocs {
    return {
      id: row.id,
      content: row.content,
      category: row.category,
      layer: row.layer as "surface" | "deep",
      weight: row.weight,
  ...
  searchFragments(query: string, options: {
    ...
    const candidates: FragmentWithAssocs[] = [];
    ...
    const selected: FragmentWithAssocs[] = [];
  ...
  private updateRecall(id: string) {
  ...
  getFragments(options: { category?: string; layer?: "surface" | "deep" } = {}): FragmentWithAssocs[] {
  ...
  getStats() {
    ...
    const layerRows = layerStmt.all() as any[];
    for (const row of layerRows) {
      by_layer[row.layer as "deep" | "surface"] = row.count;
    }

    const by_category: Record<string, number> = {};
    const catStmt = this.db.prepare(`SELECT category, COUNT(*) as count FROM fragments GROUP BY category`);
    const catRows = catStmt.all() as any[];
    ...
    const unusedResult = (unusedStmt.get(thirtyDaysAgo) as any) || {};
  ...
  compact(mode: "gentle" | "auto" | "aggressive" = "gentle", options: { unused_days?: number } = {}): { success: boolean; report: any } {
    ...
    if (mode !== "gentle") {
      ...
      const rows2 = selectStmt2.all(threshold2) as any[];
...
function loadMemory(configPath: string): MemoryConfig | null {
  try {
...
function formatMemoryForPrompt(memory: MemoryConfig): string {
...
const MEMORY_MANAGER_PATH = path.join(
...
function callMemoryManager(args: string[]): { success: boolean; data?: unknown; error?: string } {
  try {
...
export const opencodeMemoryPlugin: Plugin = async (input: PluginInput) => {
  const homeDir = process.env.USERPROFILE || process.env.HOME || "";
    ...
    tool: {
      memory: tool({
        ...
        async execute(args) {
          ...
          if (category) cmdArgs.push(category);
          if (key) cmdArgs.push(key);
          if (value) cmdArgs.push(value);
      ...
      memory_fragment_view: tool({
        ...
        async execute(args) {
          ...
          const fragments = fragmentDB.getFragments({ category, layer: layer as "surface" | "deep" | undefined });
